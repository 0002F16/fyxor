...
function Section({ title, children, onRegenerate, onRemove, onMoveUp, onMoveDown, busy }: {
  ...
  return (
    <section className="group/sec mt-4 first:mt-0">
      ...
      <div className="mt-2 text-[12.5px] leading-relaxed text-ink">{children}</div>
    </section>
  );
}
...
export function CvDocument({ cv, headline: headlineProp, editable = false, onChange, onCommitHeadline, onRegenerate, busy = false }: {
  ...
  const PAGE_HEIGHT_PX = 1123; // A4 height at 96dpi
  const pageBreaks: number[] = [];
  if (editable) for (let y = PAGE_HEIGHT_PX; y < contentHeight; y += PAGE_HEIGHT_PX) pageBreaks.push(y);
      ...
      <Section title="Experience" {...moveProps("experience")}>
        <div className="space-y-3.5">
          {cv.experiences.map((experience) => (
            <div className="group/sec relative" key={experience.id}>
      ...
      <Section title="Education" {...moveProps("education")}>
        <div className="space-y-3.5">
          {education.map((entry) => editable ? (
            <div className="group/sec relative" key={entry.id}>
              <div className="flex items-baseline justify-between gap-3">
  ...
  return (
    <article ref={pageRef} className={editable
      ? "cv-document cv-canvas relative mx-auto w-[794px] max-w-full bg-white px-[64px] py-[72px] font-sans text-ink shadow-soft"
      : "cv-document mx-auto w-full max-w-[640px] bg-white px-8 py-9 font-sans text-ink shadow-soft"}>
      ...
      <header className="group/sec relative border-b-2 border-deep pb-3 text-center">
        ...
        {editable ? (
          <div className="mt-2.5 flex flex-wrap items-center justify-center gap-x-3.5 gap-y-1 text-[11.5px] text-muted">
